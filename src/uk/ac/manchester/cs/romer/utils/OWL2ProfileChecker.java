...
public class OWL2ProfileChecker {
	 ...
	 */
	public OWL2ProfileChecker(OWLOntology ont) {
		this.ont = ont;
	...
	private boolean insertDeclarations(OWLOntology ont) {
		...
		for(OWLEntity e : ont.getSignature()) {
			List<OWLOntologyChange> l = ont.getOWLOntologyManager().applyChange(new AddAxiom(ont, df.getOWLDeclarationAxiom(e)));
			if(l.size() > 0)
				insertedSome = true;
		}
		return insertedSome;
	...
	public static void main(String[] args) {
		try {
//			FileWriter out = new FileWriter(new File("/Users/rafa/Desktop/rafa/profiles.csv"), true);
			
			for(String s : args) {
				OWLOntologyManager man = OWLManager.createOWLOntologyManager();
				
				OWLOntology ont = man.loadOntologyFromOntologyDocument(new File(s));
				System.out.println("Loaded ont " + s);
				
				
				
				OWL2ProfileChecker checker = new OWL2ProfileChecker(ont);
				System.out.println(checker.getCompleteProfileReport(s, true));
//				out.flush();
